...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleRegisterSubmit(event) {
        ...
        console.log('Данные формы:', { ...formData, password: '[СКРЫТО]', confirmPassword: '[СКРЫТО]' });
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                FormUtils.showNotification('Регистрация успешна! Добро пожаловать!', 'success');
            ...
            FormUtils.showNotification('Ошибка соединения с сервером. Попробуйте позже.', 'error');
        } finally {
    ...
    function validateForm(data) {
        ...
        if (!data.lastName || data.lastName.length < 1) {
            FormUtils.showFieldError('lastName', 'Фамилия обязательна для заполнения');
            isValid = false;
        }
        ...
        if (!data.password) {
            ...
            if (!passwordValidation.isValid) {
                FormUtils.showFieldError('password', passwordValidation.errors.join(', '));
                isValid = false;
            }
        }
        ...
        if (data.username && data.username.length < 3) {
            FormUtils.showFieldError('username', 'Логин должен содержать минимум 3 символа');
            isValid = false;
        }
